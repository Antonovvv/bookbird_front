export default { 
	...
	created() {
		...
		this.contentHeight = this.sysHeight - uni.getSystemInfoSync().statusBarHeight - 44
	...
	methods:{
		...
		touchMove(e) {
			...
			this.lookList[0].moveX = this.moveX
			this.lookList[0].moveY = this.moveY
			
			this.lookList[0].animation = false
			//移动图片旋转角度
		...
		_back(){
			...
			this.moveX = 0
			this.moveY = 0
			this.lookList[0].moveX = 0
			this.lookList[0].moveY = 0
			this.lookList[0].animation = true
		...
		_del(){
			...
			setTimeout(() => {
				...
				this.lookList[0].moveX = 0
				this.lookList[0].moveY = 0
				this.lookList.splice(0,1)
				if(this.lookList.length<=this.number) this.getList()
				if(this.type) {
					//#ifdef APP-PLUS
					this.delFlag = false
					//#endif
